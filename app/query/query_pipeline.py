...
import cv2
import numpy as np
import logging_config  # Ensure central logging is loaded
...
def query_image(image_path: str, device: str = 'cpu', top_k: int = 5) -> list:
    ...
    try:
        ...
        # We call process_image() which returns a tuple (composite_image, embeddings).
...
def compose_query_results(query_image_path: str, matches: list, output_folder: str, composite_filename: str, target_height: int = 224) -> str:
    ...
    Args:
        ...
        matches (list): A list of match dictionaries from the Pinecone query.
                   Each match's metadata should include an "image_url" field.
    ...
    for match in matches:
        ...
        # Use the image_url from metadata (which we ensured is stored during upsert).
    ...
    # Concatenate the query image with the match images.
